<template>
  <div class="table-container">
    ...
    <el-button type="primary" :loading="exportLoading" icon="el-icon-download" @click="exportExcel" class="margin-bottom">下载
    </el-button>
    <el-tabs type="border-card" @tab-click="handleClick" :class="widgetsconfig.displayTimewindow?'displayTimewindow':'noTimewindow'" v-model="activeTabName">
      <el-tab-pane :label="tab.label" :name="tab.index" v-for="(tab,tabIndex) in tabs" :key="tabIndex">
        <el-table :data="tableData"  :height="tableHeight"  @sort-change='tableSortChange'>
        <el-table-column
          v-for="(item, index) in tab.listTitle"
          :key="index"
          :prop="item.property"
          :label="item.label"
          sortable
...
export default {
  ...
  methods: {
    ...
    handleClick (tab, event) {
      ...
      this.list = []
      this.list = this.tabs[tab.name].list
    ...
    fn (obj, type) {
      ...
      const fnBody = isEntity ? obj.postFuncBody || 'return value' : obj.funcBody || 'return prevValue'
