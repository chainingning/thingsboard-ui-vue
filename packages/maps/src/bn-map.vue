<template>
  <div class="map-box">
    <el-card class="op-box">
      <el-collapse>
        <el-collapse-item>
<!--          <el-button @click="test">test</el-button>-->
<!--          <br>-->
          <el-button type="primary" title="编辑时间窗口" @click="editTimewindow" v-if="!useDashboardTimewindow">
            <i class="el-icon-time el-icon--left"></i>
            {{timewindow | formatConfigTime}}
          </el-button>
          <el-form ref="form" :model="form">
            <el-form-item label="实体名">
              <el-select v-model="form.selectValue" @change="changeSelect"
                         style="width: 200px" multiple filterable clearable>
                <el-option v-for="(item, index) in form.selectOptionsList"
                           :key="index"
                           :label="item.name"
                           :value="item.id">
                </el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="轨迹线">
              <el-switch v-model="form.isShowTrack" @change="changeSwitch"></el-switch>
            </el-form-item>
            <el-form-item label="显示名称">
              <el-switch v-model="form.isShowName" @change="changeSwitch"></el-switch>
            </el-form-item>
          </el-form>
        </el-collapse-item>
      </el-collapse>
    </el-card>
    <BnBaseMap @finishLoadMap="finishLoadMap" :init="initMapData"></BnBaseMap>
    <time-window-dialog ref="timeWindowDialog" @submit="submitTimewindow"></time-window-dialog>
  </div>
</template>

<script>
import { formatTimestamp, formatTimeMs, randomColor } from '@/utils/index'
import timeWindowDialog from '@/views/dashboards/components/time-window-dialog'
export default {
  name: 'BnMap',
  data () {
    return {
      style: '',
      form: {
        selectValue: [],
        selectOptionsList: [],
        isShowTrack: true,
        isShowName: true
      },
      initMapData: {
        centerLongitude: 116.41005240556589,
        centerLatitude: 39.50980350216803,
        zoom: 15
      },
      loading: true,
      isRendering: false,
      map: null,
      pointLayer: null, // 点图层
      lineLayer: null, // 线图层
      textLayer: null, // 文本图层
      polygonLayer: null, // 多边形图层
      pathsList: {}, // 每个设备的轨迹
      infoList: {}, // 每个设备的信息
      settings: {
        defaultCenterPosition: '116.410052,39.509803',
        markerImages: [
          'data:image/png;base64,'
        ],
        lngKeyName: 'longitude',
        latKeyName: 'latitude',
        polygonKeyName: 'coordinates'
      },
      useDashboardTimewindow: true, // 使用应用库时间
      timewindow: {}, // 时间窗口
      isTsData: true, // 是否实时数据
      cmdIdList: [],
      functionIdList: [], // 要执行的函数列表
      cmdIdToEntityId: [], // cmdId对应实体id
      aliasToEntity: {}, // 实体别名对应实体id
      entityIdToFunction: {}, // 实体id对应属性函数
      aliasToFunction: {}, // 实体别名对应属性函数
      entityIdToOptions: {}, // 实体id对应属性：color轨迹颜色 name设备名
      entityIdToPolygon: {}, // 实体id对应多边形
      timer: null
    }
  },
  components: {
    timeWindowDialog
  },
  computed: {
    getWsData () {
      return this.$store.state.websocket.websocketData.filter(item => this.cmdIdList.includes(item.subscriptionId))
    },
    selectEntityList () { // 选中显示的实体列表
      const list = []
      this.form.selectValue.forEach(item => {
        list.push(...this.aliasToEntity[item])
      })
      return list
    }
  },
  inject: ['widgetInfo', 'config', 'id'],
  methods: {
    test () {
      console.log(this.entityIdToFunction, this.infoList, this.pathsList)
    },
    initMap () {
      const { settings } = this.config
        ? this.config.configuration.widgets[this.id].config
        : JSON.parse(this.widgetInfo.descriptor.defaultConfig)
      this.settings = Object.assign({}, settings)
      this.initMapData = {
        centerLongitude: this.settings.defaultCenterPosition.split(',')[0],
        centerLatitude: this.settings.defaultCenterPosition.split(',')[1],
        zoom: 15
      }
    },
    renderMap ([util, BNSymbol, BNFactory, BNDirectorys]) {
      if (this.loading || this.isRendering) return
      this.$nextTick(async () => {
        this.isRendering = true
        if (JSON.stringify(this.pathsList) === '{}') {
          this.isRendering = false
          return
        }
        if (!this.lineLayer) {
          this.lineLayer = this.map.map.creatGraphicsLayer({ id: 'line' })
        }
        if (!this.pointLayer) {
          this.pointLayer = this.map.map.creatGraphicsLayer({ id: 'point' })
        }
        if (!this.textLayer) {
          this.textLayer = this.map.map.creatGraphicsLayer({ id: 'text' })
        }
        if (!this.polygonLayer) {
          this.polygonLayer = this.map.map.creatGraphicsLayer({ id: 'polygon' })
        }
        this.map.map.addGraphicsLayer(this.polygonLayer)
        this.map.map.addGraphicsLayer(this.lineLayer)
        this.map.map.addGraphicsLayer(this.textLayer)
        this.map.map.addGraphicsLayer(this.pointLayer)
        const BNGeometry = new BNFactory('BNGeometry', this.map.map)
        const BNmapUtil = new BNFactory('BNmapUtil', this.map.map)
        const bnSymbol = new BNSymbol()
        // 创建窗体工具
        const windows = new BNFactory('BNWindow')
        // 绘制多边形
        const polygonList = []
        for (const id in this.entityIdToPolygon) {
          const xyList = await this.doLatLngTransformMapXY(BNmapUtil, this.entityIdToPolygon[id])
          const list = []
          for (const obj of xyList) {
            list.push([obj.x, obj.y])
          }
          polygonList.push(list)
        }
        const polygonOptions = {
          rings: polygonList,
          spatialReference: this.map.map.spatialReference
        }
        const polygon = BNGeometry.createPolygon(polygonOptions)
        const simpleLineSymbol = bnSymbol.simpleLineSymbol('solid', 'red', 2)
        const polygonSymbol = bnSymbol.polygonSymbol(
          'solid',
          simpleLineSymbol,
          'rgb(255, 255, 255, 1)'
        )
        const polygonGraphics = this.map.map.graphic(polygon, polygonSymbol)
        this.polygonLayer.clear()
        this.polygonLayer.add(polygonGraphics)
        // 绘制轨迹
        for (const id in this.pathsList) {
          if (!this.selectEntityList.includes(id)) {
            for (const i in this.lineLayer.graphics) {
              if (this.lineLayer.graphics[i].id === id) {
                this.lineLayer.remove(this.lineLayer.graphics[i])
              }
            }
            for (const i in this.pointLayer.graphics) {
              if (this.pointLayer.graphics[i].id === id) {
                this.pointLayer.remove(this.pointLayer.graphics[i])
              }
            }
            for (const i in this.textLayer.graphics) {
              if (this.textLayer.graphics[i].id === id) {
                this.textLayer.remove(this.textLayer.graphics[i])
              }
            }
            continue
          }
          const paths = this.pathsList[id]
          const xyList = await this.doLatLngTransformMapXY(BNmapUtil, paths)
          if ((xyList.length > 1) && (this.form.isShowTrack)) {
            const linePaths = []
            for (const obj of xyList) {
              linePaths.push([obj.x, obj.y])
            }
            const lineJson = {
              paths: [linePaths],
              spatialReference: this.map.map.spatialReference
            }
            const line = BNGeometry.createPolyline(lineJson)
            const lineSymbol = bnSymbol.simpleLineSymbol('solid', this.entityIdToOptions[id].color, 2)
            const lineGraphics = this.map.map.graphic(line, lineSymbol)
            lineGraphics.id = id
            for (const i in this.lineLayer.graphics) {
              if (this.lineLayer.graphics[i].id === id) {
                this.lineLayer.remove(this.lineLayer.graphics[i])
              }
            }
            this.lineLayer.add(lineGraphics)
          } else {
            for (const i in this.lineLayer.graphics) {
              if (this.lineLayer.graphics[i].id === id) {
                this.lineLayer.remove(this.lineLayer.graphics[i])
              }
            }
          }
          const xy = xyList[xyList.length - 1]
          const point = BNGeometry.createPoint(xy.x, xy.y, this.map.map.spatialReference)
          const picSymbol = bnSymbol.PictureMarkerSymbol(this.settings.markerImages[0], 40, 40)
          const font = bnSymbol.font('16', bnSymbol.STYLE_ITALIC, bnSymbol.VARIANT_NORMAL, bnSymbol.WEIGHT_BOLD, 'Courier')
          const textSymbol = bnSymbol.textSymbol(this.entityIdToOptions[id].name, font, '#000000')
          textSymbol.yoffset = 30
          const pointGraphics = this.map.map.graphic(point, picSymbol)
          pointGraphics.id = id
          const textGraphics = this.map.map.graphic(point, textSymbol)
          textGraphics.id = id
          for (const i in this.pointLayer.graphics) {
            if (this.pointLayer.graphics[i].id === id) {
              this.pointLayer.remove(this.pointLayer.graphics[i])
            }
          }
          for (const i in this.textLayer.graphics) {
            if (this.textLayer.graphics[i].id === id) {
              this.textLayer.remove(this.textLayer.graphics[i])
            }
          }
          this.pointLayer.add(pointGraphics)
          if (this.form.isShowName) {
            this.textLayer.add(textGraphics)
          }
        }
        this.pointLayer.on('click', (e) => {
          const id = e.graphic.id
          const item = this.infoList[id][this.infoList[id].length - 1]
          let html = ''
          for (const key in item) {
            html += `${key}: ${item[key]}</br>`
          }
          windows.setContent(html)
          windows.setTitle(this.entityIdToOptions[id].name)
          windows.resize(280, 360)
          windows.show(e.mapPoint)
        })
        this.isRendering = false
      })
    },
    startRenderMap () {
      if (!this.map) return
      if (!this.map.argList) return
      this.renderMap(this.map.argList)
    },
    finishLoadMap (val) {
      this.loading = false
      this.map = val
      this.startRenderMap()
    },
    editTimewindow () {
      this.$refs.timeWindowDialog.openDialog(this.timewindow)
    },
    submitTimewindow (val) {
      this.timewindow = val
      this.restart()
    },
    async restart () {
      clearInterval(this.timer)
      this.timer = null
      this.$store.dispatch('websocketUnsubscribe', this.cmdIdList)
      this.pathsList = {}
      this.isTsData = (!this.timewindow) || (JSON.stringify(this.timewindow) === '{}') || (this.timewindow.selectedTab === 0)
      this.setAliasToEntity()
      await this.sendWsData()
      this.getDevicesName()
      this.setPos()
      this.loop()
    },
    changeSelect () {
      this.startRenderMap()
    },
    changeSwitch () {
      this.startRenderMap()
    },
    async sendWsData () {
      this.cmdIdToEntityId = {}
      const data = {
        attrSubCmds: [],
        historyCmds: [],
        tsSubCmds: []
      }
      const { datasources } = this.config
        ? this.config.configuration.widgets[this.id].config
        : JSON.parse(this.widgetInfo.descriptor.defaultConfig)
      this.setSelectOptionsList(datasources)
      this.functionIdList = []
      this.entityIdToFunction = {}
      this.aliasToFunction = {}
      this.entityIdToOptions = {}
      datasources.forEach(item => {
        if (item.type === 'entity') { // 实体类型
          const entityInfo = this.config.configuration.entityAliases[item.entityAliasId]
          const isMultiEntity = entityInfo.filter[entityInfo.filter.type] instanceof Array
          if (isMultiEntity) {
            item.entityList = entityInfo.filter[entityInfo.filter.type].map(id => {
              return {
                entityType: entityInfo.filter.entityType,
                id
              }
            })
          } else {
            item.entityList = [{
              entityType: entityInfo.filter[entityInfo.filter.type].entityType,
              id: entityInfo.filter[entityInfo.filter.type].id
            }]
          }
          const obj = {}
          item.dataKeys.forEach(keyItem => {
            const fnBody = keyItem.postFuncBody ? keyItem.postFuncBody : 'return value'
            obj[keyItem.name] = Function.call(this, 'time', 'value', fnBody)
          })
          item.entityList.forEach(entityItem => {
            this.entityIdToFunction[entityItem.id] = obj
            this.entityIdToOptions[entityItem.id] = {
              color: randomColor(),
              name: entityItem.id,
              type: 'entity'
            }
          })
        } else { // 函数类型
          this.functionIdList.push(item.name)
          this.aliasToEntity[item.name] = [item.name]
          item.entityList = [{
            id: item.name
          }]
          const obj = {}
          item.dataKeys.forEach(keyItem => {
            const fnBody = keyItem.funcBody ? keyItem.funcBody : 'return value'
            obj[keyItem.label] = Function.call(this, 'time', 'value', fnBody)
          })
          item.entityList.forEach(entityItem => {
            this.entityIdToFunction[entityItem.id] = obj
            this.entityIdToOptions[entityItem.id] = {
              color: randomColor(),
              name: entityItem.id,
              type: 'function'
            }
          })
        }
      })
      const tsSubCmds = []
      const historyCmds = []
      let cmdIndex = 0
      if (this.isTsData) { // 发送的实时数据
        datasources.forEach(item => {
          if (item.type === 'function') return
          const keysList = []
          item.dataKeys.forEach(keyItem => {
            keysList.push(keyItem.name)
          })
          const interval = this.timewindow.realtime ? this.timewindow.realtime.interval : 1000
          const timewindowMs = this.timewindow.realtime ? this.timewindow.realtime.timewindowMs : 60000
          const agg = this.timewindow.aggregation ? this.timewindow.aggregation.type : 'NONE'
          const limit = this.timewindow.aggregation ? this.timewindow.aggregation.limit : 50
          const startTs = new Date().getTime() - (timewindowMs + interval)
          item.entityList.forEach(entityItem => {
            const obj = {
              entityId: entityItem.id,
              entityType: entityItem.entityType,
              keys: keysList.join(','),
              agg,
              interval,
              timeWindow: timewindowMs + interval,
              limit: agg === 'NONE' ? limit : timewindowMs / interval + 1,
              startTs
            }
            tsSubCmds.push(obj)
            this.cmdIdToEntityId[cmdIndex] = entityItem.id
            cmdIndex++
          })
        })
      } else { // 发送的历史数据
        const interval = this.timewindow.history ? this.timewindow.history.interval : 1000
        const timewindowMs = this.timewindow.history ? this.timewindow.history.timewindowMs : 60000
        const agg = this.timewindow.aggregation ? this.timewindow.aggregation.type : 'NONE'
        const limit = this.timewindow.aggregation ? this.timewindow.aggregation.limit : 50
        const historyType = this.timewindow.history ? this.timewindow.history.historyType : 0
        const nowTime = new Date().getTime()
        const startTs = historyType ? this.timewindow.history.fixedTimewindow.startTimeMs : nowTime - timewindowMs
        const endTs = historyType ? this.timewindow.history.fixedTimewindow.endTimeMs : nowTime
        datasources.forEach(item => {
          if (item.type === 'function') return
          const keysList = []
          item.dataKeys.forEach(keyItem => {
            keysList.push(keyItem.name)
          })
          item.entityList.forEach(entityItem => {
            const obj = {
              entityId: entityItem.id,
              entityType: entityItem.entityType,
              keys: keysList.join(','),
              agg,
              startTs,
              endTs,
              interval,
              limit: agg === 'NONE' ? limit : (endTs - startTs) / interval
            }
            historyCmds.push(obj)
            this.cmdIdToEntityId[cmdIndex] = entityItem.id
            cmdIndex++
          })
        })
      }
      data.tsSubCmds = tsSubCmds
      data.historyCmds = historyCmds
      this.cmdIdList = await this.$store.dispatch('websocketsend', data) || []
    },
    setPos () {
      const limit = (this.timewindow && this.timewindow.aggregation) ? this.timewindow.aggregation.limit : 50
      if (this.isTsData) { // 实时数据
        this.getWsData.forEach(item => {
          const index = this.cmdIdList.indexOf(item.subscriptionId)
          const deviceId = this.cmdIdToEntityId[index]
          if ((!this.pathsList[deviceId]) || (this.pathsList[deviceId].length === 0)) {
            const longitude = item.data[this.settings.lngKeyName]
            if (item.data[this.settings.polygonKeyName]) {
              this.addPolygon(deviceId, item.data[this.settings.polygonKeyName])
            }
            if (longitude && longitude.length > 0) {
              // this.pathsList[deviceId] = []
              this.infoList[deviceId] = []
              for (let i = item.data[this.settings.lngKeyName].length - 1; i >= 0; i--) {
                const obj = {
                  timeStamp: longitude[i][0]
                }
                for (const key in item.data) {
                  const fn = this.entityIdToFunction[deviceId][key]
                  obj[key] = fn(item.data[key][i][0], item.data[key][i][1])
                }
                this.infoList[deviceId].push(obj)
                this.infoListToPathsList(deviceId)
              }
              this.startRenderMap()
            }
          } else {
            if (this.timewindow.aggregation.type === 'NONE') {
              for (const key in this.infoList) {
                const nowTime = this.infoList[key][this.infoList[key].length - 1].timeStamp
                this.infoList[key] = this.infoList[key].filter(item => {
                  return nowTime - item.timeStamp <= this.timewindow.realtime.timewindowMs
                })
                this.infoListToPathsList(key)
              }
            }
            if (item.data[this.settings.polygonKeyName]) {
              this.addPolygon(deviceId, item.data[this.settings.polygonKeyName])
            }
            const len = this.infoList[deviceId].length
            const newTimeStamp = item.data[this.settings.lngKeyName][0][0]
            const oldTimeStamp = this.infoList[deviceId][len - 1].timeStamp
            const limit2 = this.timewindow.realtime.timewindowMs / this.timewindow.realtime.interval + 1
            if (newTimeStamp !== oldTimeStamp) {
              if (len > (limit < limit2 ? limit : limit2)) {
                this.infoList[deviceId].shift()
              }
              const obj = {
                timeStamp: item.data[this.settings.lngKeyName][0][0]
              }
              for (const key in item.data) {
                obj[key] = item.data[key][0][1]
              }
              this.infoList[deviceId].push(obj)
              this.infoListToPathsList(deviceId)
              this.startRenderMap()
            }
          }
        })
        this.functionIdList.forEach(id => {
          const longFn = this.entityIdToFunction[id][this.settings.lngKeyName]
          const latFn = this.entityIdToFunction[id][this.settings.latKeyName]
          const time = new Date().getTime()
          const longitude = longFn(time)
          const latitude = latFn(time)
          const newPos = [Number(longitude), Number(latitude)]
          const obj = {
            timeStamp: time
          }
          obj[this.settings.lngKeyName] = Number(longitude)
          obj[this.settings.latKeyName] = Number(latitude)
          if (!this.infoList[id]) {
            this.pathsList[id] = []
            this.infoList[id] = []
            this.pathsList[id].push(newPos)
            this.infoList[id].push(obj)
            this.startRenderMap()
          } else {
            if (this.infoList[id].length >= limit) {
              this.infoList[id].shift()
              this.pathsList[id].shift()
            }
            const len = this.infoList[id].length
            const oldTime = this.infoList[id][len - 1].timeStamp
            if (time !== oldTime) {
              this.pathsList[id].push(newPos)
              this.infoList[id].push(obj)
              this.startRenderMap()
            }
          }
        })
      } else { // 历史数据
        this.getWsData.forEach(item => {
          const index = this.cmdIdList.indexOf(item.subscriptionId)
          const deviceId = this.cmdIdToEntityId[index]
          const longitude = item.data[this.settings.lngKeyName]
          if (item.data[this.settings.polygonKeyName]) {
            this.addPolygon(deviceId, item.data[this.settings.polygonKeyName])
          }
          if (longitude && longitude.length > 0) {
            this.pathsList[deviceId] = []
            this.infoList[deviceId] = []
            for (let i = item.data[this.settings.lngKeyName].length - 1; i >= 0; i--) {
              const obj = {
                timeStamp: longitude[i][0]
              }
              for (const key in item.data) {
                const fn = this.entityIdToFunction[deviceId][key]
                obj[key] = fn(item.data[key][i][0], item.data[key][i][1])
              }
              this.infoList[deviceId].push(obj)
              this.infoListToPathsList(deviceId)
            }
          }
        })
        if (JSON.stringify(this.infoList) !== '{}') {
          this.startRenderMap()
        }
      }
    },
    infoListToPathsList (id) { // id: 设备id 不传值默认全部
      if (id) {
        const list = []
        for (const obj of this.infoList[id]) {
          list.push([obj[this.settings.lngKeyName], obj[this.settings.latKeyName]])
        }
        this.pathsList[id] = list
      } else {
        this.pathsList = {}
        for (const id in this.infoList) {
          const list = []
          for (const obj of this.infoList[id]) {
            list.push([Number(obj[this.settings.lngKeyName]), Number(obj[this.settings.latKeyName])])
          }
          this.pathsList[id] = list
        }
      }
    },
    addPolygon (id, arr) {
      const listStr = arr[0][1]
      const list = JSON.parse(listStr)
      list.push(list[0])
      this.entityIdToPolygon[id] = list
    },
    doLatLngTransformMapXY (util, coordinateArray) { // 经纬度转地图坐标
      return new Promise((resolve, reject) => {
        util.doLatLngTransformMapXY(coordinateArray, 4326, this.map.map.spatialReference, data => {
          resolve(data)
        })
      })
    },
    setSelectOptionsList (datasources) {
      this.form.selectOptionsList = []
      for (const data of datasources) {
        this.form.selectOptionsList.push({ // 函数不存在entityAliasId
          name: data.entityAliasId ? this.config.configuration.entityAliases[data.entityAliasId].alias : data.name,
          id: data.entityAliasId ? data.entityAliasId : data.name
        })
      }
    },
    setAliasToEntity () {
      this.aliasToEntity = {}
      if (!this.config) return
      for (const key in this.config.configuration.entityAliases) {
        const item = this.config.configuration.entityAliases[key]
        const isMultiEntity = item.filter[item.filter.type] instanceof Array
        if (isMultiEntity) {
          this.aliasToEntity[item.id] = item.filter[item.filter.type]
        } else {
          this.aliasToEntity[item.id] = [item.filter[item.filter.type].id]
        }
      }
    },
    async getDevicesName () {
      const pList = []
      for (const key in this.entityIdToOptions) {
        if (this.entityIdToOptions[key].type === 'function') continue
        pList.push(new Promise(async resolve => {
          const res = await this.$api.getItemDeviceInfo(key)
          this.entityIdToOptions[key].name = res.data.name
          resolve()
        }))
      }
      await Promise.all(pList)
    },
    loop () {
      this.timer = setInterval(() => {
        this.setPos()
      }, 1000)
    }
  },
  filters: {
    formatConfigTime (val) {
      let str = ''
      if (val.selectedTab === 1) {
        str = '历史 - '
        if (val.history.historyType === 1) {
          str = str + ' 从 ' + formatTimestamp(val.history.fixedTimewindow.startTimeMs) + ' 到 ' + formatTimestamp(val.history.fixedTimewindow.endTimeMs)
        } else {
          str = str + '最后 ' + formatTimeMs(val.history.timewindowMs)
        }
      } else {
        str = '实时 - 最后 ' + (val.realtime ? formatTimeMs(val.realtime.timewindowMs) : '1分')
      }
      return str
    }
  },
  async created () {
    this.useDashboardTimewindow = this.config ? this.config.configuration.widgets[this.id].config.useDashboardTimewindow : true
    this.timewindow = this.config ? (this.useDashboardTimewindow ? this.config.configuration.timewindow : this.config.configuration.widgets[this.id].config.timewindow) : {}
    this.isTsData = (!this.timewindow) || (JSON.stringify(this.timewindow) === '{}') || (this.timewindow.selectedTab === 0)
    this.setAliasToEntity()
    await this.sendWsData()
  },
  mounted () {
    this.getDevicesName()
    this.initMap()
    this.setPos()
    this.loop()
  },
  beforeDestroy () {
    clearInterval(this.timer)
    this.timer = null
    this.$store.dispatch('websocketUnsubscribe', this.cmdIdList)
  }
}
</script>

<style lang="scss" scoped>
  .map-box {
    position: relative;
    width: 100%;
    height: 100%;
    .op-box {
      position: absolute;
      z-index: 99;
    }
  }
</style>
