  ...
  methods: {
    ...
    async deleteMore () {
      this.$confirm('注意,确认后所有选中的属性都会被删除', `确定要删除${this.selection.length}属性吗?`, {
        ...
        await this.$api.deletePluginsTelemetry('TENANT', this.tenantId, {
    ...
    submit () {
      this.$refs.form.validate(async valid => {
        ...
        await this.$api.postPluginsTelemetry('TENANT', this.tenantId, params, 'SERVER_SCOPE')
        this.getList()
        this.visible = false
      })
    },
    async getList () {
      const result = await this.$api.getAttribute('TENANT', this.tenantId, this.listQuery.scope)
