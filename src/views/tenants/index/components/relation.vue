  ...
  methods: {
    ...
    submit () {
      this.$refs.form.validate(async valid => {
        ...
        const { type, id, additionalInfo } = this.form
    ...
    async remoteMethod (query) {
      ...
      switch (this.form.entityType) {
        case 'ENTITY_VIEW':
          result = await this.$api.getTenantsList(Object.assign({
            sortProperty: 'title'
          }, params))
          break
        default:
    ...
    async getList () {
      const result = await this.$api.getRelationsInfo({
        [`${this.listQuery.id}Id`]: this.tenantId,
        [`${this.listQuery.id}Type`]: 'TENANT'
      })
