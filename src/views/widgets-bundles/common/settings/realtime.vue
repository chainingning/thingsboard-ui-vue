  ...
  <icloud-dialog
    ...
    <el-tabs type="border-card">
      <el-tab-pane label="数据">
        ...
        <el-row>
          <el-col :span="12">
            <el-checkbox v-model="useDashboardTimewindow">使用应用的时间窗口</el-checkbox>
          </el-col>
          <el-col :span="12">
            <span>时间窗口</span>
            <el-input v-model="activeTabName"></el-input>
            <el-button type="primary" title="编辑时间窗口" @click="editTimeDialog" :disabled="useDashboardTimewindow"><i class="el-icon-time el-icon--left"></i>{{configTimewindow | formatConfigTime}}</el-button>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-checkbox v-model="displayTimewindow" :disabled="useDashboardTimewindow">显示时间窗口</el-checkbox>
          </el-col>
        </el-row>
        -->
        <el-collapse v-model="activeName">
          <el-collapse-item title="数据源（只允许一个）" name="1">
            ...
            <el-button type="primary" @click="addType" v-if="list.length == 0">添加</el-button>
          </el-collapse-item>
        </el-collapse>
      </el-tab-pane>
      <el-tab-pane label="设置">
        <el-form ref="settings" :model="settings" :rules="settingsRules">
          ...
          <el-row :gutter="20">
            <el-col :span="12">
              <el-form-item label="特殊符号展示值" class="margin-bottom">
                <el-input v-model="settings.units"></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="12">
              <el-form-item label="浮点数后的位数" class="margin-bottom">
                <div class="full-row">
      ...
      <el-tab-pane label="高级">
        ...
        <div class="margin-bottom">
          <el-checkbox v-model="highSettings.showTimestamp">Display timestamp column</el-checkbox>
        </div>
        <div class="margin-bottom">
          <el-checkbox v-model="highSettings.showMilliseconds">Display timestamp milliseconds</el-checkbox>
        </div>
        -->
        <div class="margin-bottom">
          <el-checkbox v-model="highSettings.showEntityType">显示实体类型</el-checkbox>
        </div>
        <div class="margin-bottom">
          <el-checkbox v-model="highSettings.showDeviceTimestamp">显示时间戳</el-checkbox>
        </div>
        <div class="margin-bottom">
          <el-checkbox v-model="highSettings.displayPagination">启用分页</el-checkbox>
        </div>
        <div class="margin-bottom">
          <span>页面大小</span>
          <el-input v-model="highSettings.defaultPageSize" :disabled="!highSettings.displayPagination"></el-input>
        </div>
      </el-tab-pane>
    </el-tabs>
    ...
    <icloud-dialog title="数据键配置" :visible.sync="fnDialogVisible" class="dataConfigDialog">
      <el-form ref="form" :model="form" :rules="rules">
        ...
        <div class="fnName" v-if="editInfo.type === 'function' ? true : form.useFn">function (time, value, prevValue, prevTime, prevOrigValue) {</div>
        ...
        <div class="fnParams" v-if="editInfo.type === 'function' ? true : form.useFn">
          ...
          <div>value - 当前值;</div>
          <div>prevValue - 当前函数值;</div>
          <div>prevTime - 先前值的时间戳;</div>
          <div>prevOrigValue - 先前原始值;</div>
        </div>
      </el-form>
      <div class="icloud-dialog-footer" slot="footer">
  ...
  methods: {
    ...
    openDialog ({ widgetInfo, config, type, id }) {
      ...
      this.highSettings = {
        ...
        showEntityType: defaultConfig.settings.showEntityType === undefined ? true : defaultConfig.settings.showEntityType,
        showDeviceTimestamp: defaultConfig.settings.showDeviceTimestamp === undefined ? false : defaultConfig.settings.showDeviceTimestamp
      }
    ...
    editFn (fnConfig) {
      ...
      for (const key in this.form) {
        this.form[key] = key === 'useFn'
          ? (isHasOwnProperty(fnConfig, 'postFuncBody') && fnConfig.postFuncBody !== '')
          : fnConfig[key]
      }
