  ...
  <div class="alarm-container">
    ...
    <icloud-dialog title="警告详情" :visible.sync="visible">
      <el-form ref="form" :model="form">
        ...
        <el-form-item label="末次告警时间">
          <el-input readonly v-model="form.endTs"></el-input>
        </el-form-item>
        <el-form-item label="告警清除时间">
          <el-input readonly v-model="form.clearTs"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    async submit (alarmType) {
      await this.$api.postAlarmType(this.info.id.id, alarmType)
      this.getList()
      this.visible = false
    },
    openDialog (data) {
      ...
      for (const key in this.form) {
        if (['createdTime', 'endTs', 'startTs', 'clearTs'].includes(key)) {
    ...
    async getList (params) {
      ...
      try {
        ...
        const res = await this.$api.getAlarms('DEVICE', this.deviceId, {
