<template>
  <div class="attribute-container">
    <el-form :model="listQuery" size="medium" :inline="true">
      <el-form-item label="服务属性范围">
        <el-select v-model="listQuery.scope" @change="getList()">
  ...
  methods: {
    ...
    async deleteMore () {
      this.$confirm('注意,确认后所有选中的属性都会被删除', `确定要删除${this.selection.length}属性吗?`, {
        ...
        await this.$api.deletePluginsTelemetry('ENTITY_VIEW', this.entityId, {
    ...
    submit () {
      this.$refs.form.validate(async valid => {
        ...
        await this.$api.postPluginsTelemetry('ENTITY_VIEW', this.entityId, params, this.listQuery.scope)
        this.getList()
        this.visible = false
      })
    },
    async getList () {
      try {
        ...
        this.list = res.data.map(ele => Object.assign(ele, {
          updatedTime: getDate({ timestamp: ele.lastUpdateTs })
        }))
