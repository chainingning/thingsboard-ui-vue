  ...
  methods: {
    ...
    async deleteMore () {
      this.$confirm('注意,确认后所有选中的属性都会被删除', `确定要删除${this.selection.length}属性吗?`, {
        ...
        await this.$api.deletePluginsTelemetry('ASSET', this.assetId, {
    ...
    submit () {
      this.$refs.form.validate(async valid => {
        ...
        await this.$api.postPluginsTelemetry('ASSET', this.assetId, params, 'SERVER_SCOPE')
        this.getList()
        this.visible = false
      })
    },
    ...
    async getList () {
      try {
        const result = await this.$api.getAttribute('ASSET', this.assetId, this.listQuery.scope)
