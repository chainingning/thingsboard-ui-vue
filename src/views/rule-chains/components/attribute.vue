  ...
  methods: {
    ...
    async deleteMore () {
      this.$confirm('注意,确认后所有选中的属性都会被删除', `确定要删除${this.selection.length}属性吗?`, {
        ...
        await this.$api.deletePluginsTelemetry('RULE_CHAIN', this.id, {
    ...
    submit () {
      this.$refs.form.validate(async valid => {
        ...
        await this.$api.postPluginsTelemetry('RULE_CHAIN', this.id, params, this.listQuery.scope)
        this.getList()
        this.visible = false
      })
    },
    async getList () {
      try {
        const result = await this.$api.getAttribute('RULE_CHAIN', this.id, this.listQuery.scope)
